...
class FastFlow(nn.Module):
    def __init__(
        ...
        input_size = [channels[-1], int(input_size / scales[-1]), int(input_size / scales[-1])]
        output_size = self.nf_flows[-1](torch.zeros(1, *input_size))[0].cpu().detach().numpy().shape
        self.last_layer = self.nf_flows[-1]
        self.flatten = nn.Flatten()
        self.fc = nn.Linear(np.prod(output_size), 4096)
        self.last_output = nn.Linear(4096,2048)

    def forward(self, x):
        self.feature_extractor.eval()
        # print(f'first f_e:{self.feature_extractor}')
        if isinstance(
        ...
        # print(f'features : {features[0].size()}')
        # print(f'features : {features[1].size()}')
        # print(f'features : {features[2].size()}')
        for i, feature in enumerate(features):
            # print(f"i = {i}, feature = {feature.size()}")
        ...
        # print(f'outputs : {outputs[2].size()}')

        ret = {"loss": loss}

        last_output, log_jac_dets = self.last_layer(features[-1])
        last_output = self.flatten(last_output)
        # last_output = self.fc(last_output)
        # last_output = self.last_output(last_output)
